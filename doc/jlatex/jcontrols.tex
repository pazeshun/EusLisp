...
{\em obj}によって示されるコンディションをsignalizeする。最初にマッチングされたhandlerの返り値を返す。マッチングされたhandlerがないためにはnilを返す。
{\em obj}はコンディションのインスタンスか、 {\em init-args}によって初期化されるコンディションクラスである。}

\funcdesc{next-handler}{\&optional (obj lisp::*current-condition*)}{
コンディションハンドラーの中から呼ぶことによって、{\em obj}にマッチングする次のhandlerを呼び出すことが可能である。}
